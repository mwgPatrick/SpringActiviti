    ...
    $.extend({
    	...
    	table: {
            ...
            queryParams: function(params) {
				...
				var a = $.extend(curParams, $.common.formToJSON(currentId));
				return a;
            },
            // 请求获取数据后处理回调函数
    	...
    	form: {
    		...
    		reset: function(formId) {
            	var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
            	$("#" + currentId)[0].reset();
            	// Todo: btTable 中没有bootstrapTable方法
                $.btTable.bootstrapTable('refresh');
            },
            // 获取选中复选框项
        ...
        operate: {
			...
			detailTab: function (id) {
				$.modal.openTab( $.table._option.modalName+"详细信息", $.operate.detailUrl(id));
			},
            // 详细访问地址
        ...
        tree: {
        	...
        	init: function(options) {
        		var defaults = {
            		id: options.id==null?"tree":options.id,                // 属性ID
        	    ...
        	    $.get(options.url, function(data) {
        			...
        			if(options.onAsyncSuccess!=null&&options.onAsyncSuccess!=undefined){
